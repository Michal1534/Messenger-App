...
import { arrayRemove, collection, doc, DocumentData, getDoc, getDocs, updateDoc } from 'firebase/firestore'
import { Avatar, Box, Button, Divider, SectionList, Heading, HStack, Icon, Input, ScrollView, Text, Modal, Image } from 'native-base'
...
export const Friends: React.FC = () => {
    ...
    const filteredFriends = useMemo(() => Object.values(conversations).filter((conversation) => {
    ...
    return (<Box flex={1} alignItems="center" justifyContent="flex-start" paddingY={5}>
        ...
        <Box h={128} justifyContent="center">
            {isQrShown ? (
                <TouchableOpacity onPress={() => {
                    ...
                    <QRCode value={JSON.stringify({ app: 'KCK', email: user?.email! })} size={128} />
                </TouchableOpacity>
            )
                :
                (
        ...
        <Input textAlign={"center"} size={"xl"} value={filteredUsers} onChangeText={(value) => { setFilteredUsers(value) }} mt={"10px"} mb={"20px"} w={"100%"} variant="underlined" placeholder="Find your friend" />
        <SectionList
            w="100%"
            sections={groupedConversations.sort()}
            keyExtractor={(item, index) => item.user + index}
            renderSectionHeader={({ section: { title } }) => (
                <Text bg={"gray.300"} fontSize={"20px"} color={"gray.500"} pl={3}>{title}</Text>
            )}
            renderItem={({ item }) => (
                <TouchableOpacity onPress={() => {
                    ...
                    <Box w="100%" px={5} py={3} flexDirection="row" justifyContent="flex-start" alignItems="center" borderBottomWidth="1"
            ...
            ListEmptyComponent={() => <Box pt="10" alignItems="center" justifyContent="center"><Icon mb="3" as={FontAwesome5} name="grin-squint" size={24} /><Text>Unfortunately you have no friends.</Text></Box>}
        />
        <Button mt={"10px"} size="lg" onPress={() => {
