...
import { Box, VStack, Heading, Button, Avatar, Divider, Pressable, Text, Icon, Modal, ScrollView, Image } from "native-base";
...
export const Home = () => {
  ...
  return (<Box flex={1} alignItems="center" justifyContent="center" paddingX={10}>
    ...
    <Text italic color={statusColors[status]}>{status}</Text>
    <Divider my={10} bgColor="gray.300" />
    <VStack space="3" justifyContent="space-between">
      <Heading>Logged in succesfully</Heading>
      <Button size='lg' onPress={() => navigation.push('ChangePassword')} >Change password</Button>
      <Button size="lg" onPress={async () => {
        await signOut(auth)
        logOut()
      }}>Log out</Button>
    </VStack>
