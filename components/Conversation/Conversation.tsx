  ...
  return (
    <TouchableOpacity onPress={changeStatus}>
      <Box marginY={'5px'}
        ...
        <Box alignItems={'flex-end'} maxWidth={'75%'} rounded={'10px'} backgroundColor={item.isOwnMessage ? `green.${color}` : `blue.${color}`} padding='10px'>
          <Text >{item.message}</Text>
          <Text fontSize={'8px'} textAlign={'right'} italic color={'gray.400'}>{formatDistanceToNow(item.time)}</Text>
        </Box>
      </Box>
    </TouchableOpacity>
  )
...
const Footer: React.FC<{ user: string, listRef: React.RefObject<FlatList<any>> }> = ({ user, listRef }) => {
  ...
  return (
    <Box flexDirection={'row'} py={1}>
      ...
      }} backgroundColor={'transparent'} disabled={value.length === 0} startIcon={<Icon as={FontAwesome5} name="paper-plane" size={5} color={value.length > 0 ? "primary.500" : 'gray.500'} />}></Button>
    </Box>
  )
}
  ...
  return (
    <Box flex={1} safeAreaBottom backgroundColor="white">
      <FlatList
        ...
        renderItem={({item}) => <MessageCloud item={item} key={item.key} changeStatus={() => {
