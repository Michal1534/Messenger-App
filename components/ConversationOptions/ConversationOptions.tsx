...
import { MaterialTopTabNavigationOptions, MaterialTopTabScreenProps } from '@react-navigation/material-top-tabs'
...
export const ConversationOptions: React.FC = () => {
    ...
    const { params: { user, category } } = useRoute<RouteProp<ConversationProps, "ConversationOptions">>()
    ...
    return (
        <Box flex={1} p={5} justifyContent="flex-start" alignItems="center">
            ...
            <Box mb={"20px"}>
                <Input value={value} onChangeText={setValue} mb={"10px"} w={"100%"} variant="underlined" placeholder="Set nickname" />
                <Button onPress={() => { changeNickname(user, value) }}>Zmień</Button>
            </Box>
            <Heading mb={3} mt={"20px"}>Select a category</Heading>
            <Select selectedValue={friendCategory} minWidth={"100%"} placeholder="Select a category" dropdownIcon={<Icon as={FontAwesome5} name="chevron-down" size={5} />} _selectedItem={{
                bg: "gray.500",
            ...
            <Button mt={3} onPress={()=>{setShowModal(!showModal)}}>
                Check Instruction
            </Button>
        </Box>
    )
